...
def project(
    ...
    w_samples = G.mapping(torch.from_numpy(z_samples).to(device), parameters[3].repeat([w_avg_samples, 1]), truncation_psi=parameters[1], truncation_cutoff=parameters[2])
    ...
    for step in tqdm(range(num_steps)):
        ...
        synth_images = G.synthesis(ws, parameters[0])['image']
        ...
        img = (synth_images.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
        PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'test_inversion/' + str(step) + '.png')
